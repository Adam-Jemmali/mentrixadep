-- League forum: threaded replies + moderated screenshots on division_messages

ALTER TABLE public.division_messages
  ADD COLUMN IF NOT EXISTS parent_id UUID REFERENCES public.division_messages (id) ON DELETE CASCADE,
  ADD COLUMN IF NOT EXISTS thread_id UUID REFERENCES public.division_messages (id) ON DELETE CASCADE,
...
-- Backfill legacy flat messages before adding thread title constraint.
...
-- Private bucket — served via signed URLs after membership check
