...
export function tierComparisonFeatureIcon(feature: string): VocabIconName {
  ...
  if (line.includes("member rate")) return CANONICAL_MOMENTUM_ICON;
  ...
  if (line.includes("proof chain") || line.includes("trajectory")) return CANONICAL_RANK_PROOF_ICON;
  if (line.includes("loop velocity")) return CANONICAL_LOOP_REPORT_ICON;
