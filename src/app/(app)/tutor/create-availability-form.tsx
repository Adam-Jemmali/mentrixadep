  ...
  return (
    <div className="border-t border-slate-200 pt-4 mt-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 text-xs">
        <div>
          ...
          {hasCourses ? (
            <Select value={course} onValueChange={setCourse}>
              <SelectTrigger className="mt-1.5 h-9 border-slate-200 text-slate-900">
        ...
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {WEEKDAYS.map((d) => (
              <label
                key={d.value}
                className="flex cursor-pointer items-center gap-1.5 rounded border border-slate-200 bg-white px-2 py-1.5 transition-colors hover:bg-slate-50 has-[:checked]:border-slate-800 has-[:checked]:bg-slate-50"
              >
                <input
                  type="checkbox"
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-slate-100 bg-slate-50/80 px-3 py-2.5">
          <div>
            <p className="text-slate-800 font-medium">Repeat weekly</p>
        ...
        {recurring && (
          <div>
            <Label className="text-slate-700">Weeks to repeat</Label>
            <Input
              type="number"
              min={1}
              max={52}
              className="mt-1.5 h-9 border-slate-200"
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label className="text-slate-700">Price (USD / session)</Label>
            <div className="relative mt-1.5">
              ...
              <Input
                ...
                min={1}
                step={1}
                className="h-9 border-slate-200 pl-6"
                value={price}
          ...
          <div>
            <Label className="text-slate-700">Max learners</Label>
            <Input
              type="number"
              disabled
              value={1}
              className="mt-1.5 h-9 border-slate-200 bg-slate-50 text-slate-600"
              readOnly
            />
            <p className="text-[10px] text-slate-400 mt-1">Group sessions later</p>
          </div>
        </div>
