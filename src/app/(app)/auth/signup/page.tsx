    ...
    if (!parsed.success) {
      setError(
        parsed.error.issues[0]?.message ??
          "Please complete signup details correctly, including age confirmation."
      );
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <Input
            ...
            autoComplete="email"
          />
          {universityEmailHint(emailInput) === "university" && (
            <p className="mt-2 text-sm text-emerald-700 flex items-center gap-1.5" role="status">
              <span aria-hidden>✓</span> University email detected
            </p>
          )}
          {universityEmailHint(emailInput) === "personal" && emailInput.includes("@") && (
            <p className="mt-2 text-sm text-slate-600 flex items-center gap-1.5" role="status">
              <span aria-hidden>ℹ️</span> Personal email — works fine!
            </p>
          )}
        </div>
        ...
        <Button type="submit" className="w-full mt-4" disabled={loading || !ageConfirmed}>
          {loading ? "Creating account…" : "Sign up"}
        </Button>

        <label className="mt-1 flex items-start gap-2 text-xs text-slate-500">
          <input
            type="checkbox"
