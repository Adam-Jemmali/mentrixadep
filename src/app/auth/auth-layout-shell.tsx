...
const PHRASES = [
  ...
  "The game is free forever. Book a Guide only when the wall is real.",
  "Quest, duel, and climb. Every action moves your rank. Every rank is earned in public.",
] as const;
...
export default function AuthLayoutShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2 bg-slate-950 text-slate-100">
      {showLeftPanel && (
        <div className="hidden lg:flex flex-col justify-between p-12 bg-[linear-gradient(180deg,#0b1220_0%,#111827_50%,#1e1b4b_100%)] text-white relative overflow-hidden">
      ...
      <div
        className={cn(
          "flex items-center justify-center bg-[linear-gradient(180deg,#0f172a_0%,#111827_100%)]",
          isSelectRole ? "min-h-screen p-4 sm:p-6 lg:p-8" : "p-8 lg:p-16",
        )}
      >
        <div
          className={cn("w-full space-y-4 stagger-children", isSelectRole ? "max-w-6xl" : "max-w-sm")}
          id="auth-form-wrapper"
        >
          <div className="flex justify-center pb-1 auth-logo-slot">
            <Link
              href="/"
              className="rounded-xl outline-none ring-offset-2 ring-offset-slate-950 focus-visible:ring-2 focus-visible:ring-indigo-400/60"
            >
