      ...
      <Tabs defaultValue="match" className="w-full">
        ...
        <TabsContent value="match" className="mt-4">
          <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm space-y-4">
            ...
            {queuePhase === "waiting" ? (
              <div className="rounded-xl border border-mentrixa-200 bg-mentrixa-50/60 px-4 py-3 text-sm text-slate-700">
                <span className="font-medium text-slate-900">Searching in </span>
                <span className="text-mentrixa-800">{activeDivisionLabel}</span>
                <span className="text-slate-500">
                  {" "}
                  — cancel to change subject.
                </span>
              </div>
            ) : (
              <>
