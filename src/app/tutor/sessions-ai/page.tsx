...
function studioSessionsKey(
  sessions: { id: string; aiPackage: unknown }[],
): string {
  return sessions
    .map((s) => `${s.id}:${s.aiPackage ? "1" : "0"}`)
    .join("|");
}
...
export default async function TutorStudioPage({ searchParams }: StudioPageProps) {
  ...
  const generatedCount = sessions.filter((session) => session.aiPackage).length;
      ...
      <header className="flex items-center justify-between mb-6 border-b border-[#E2E8F0] pb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            AI-powered packages for every session you teach.
          </p>
        </div>
